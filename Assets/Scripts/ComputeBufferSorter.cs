...
public class ComputeBufferSorter
{
    ...
    private readonly uint[] _unsortedKeysLocalData = new uint[Constants.DATA_ARRAY_COUNT];
    private readonly uint[] _sortedKeysLocalData = new uint[Constants.DATA_ARRAY_COUNT];
    ...
    public ComputeBufferSorter(int dataLength, 
        ComputeBuffer keys, 
        ...
        ComputeShader scanShader)
    {
        ...
        _sortedBlocksKeysData = new ComputeBuffer(Constants.DATA_ARRAY_COUNT, sizeof(uint));
        _sortedBlocksValuesData = new ComputeBuffer(Constants.DATA_ARRAY_COUNT, sizeof(uint));
        _offsetsData = new ComputeBuffer(Constants.BUCKET_SIZE * Constants.BLOCK_SIZE, sizeof(uint));
        _sizesData = new ComputeBuffer(Constants.BUCKET_SIZE * Constants.BLOCK_SIZE, sizeof(uint));
        _sizesPrefixSumData = new ComputeBuffer(Constants.BLOCK_SIZE / (Constants.THREADS_PER_BLOCK / Constants.BUCKET_SIZE), sizeof(uint));
        ...
        uint[] sortedKeys = new uint[_dataLength];
        _keys.GetData(sortedKeys);
        string before = string.Empty;
        foreach (uint code in sortedKeys)
            before += code.ToString() + ", ";
    ...
    public void Sort()
    {
        ...
        // PrintData();
        uint[] sortedKeys = new uint[_dataLength];
        _sortedBlocksKeysData.GetData(sortedKeys);
        string before = string.Empty;
        foreach (uint code in sortedKeys)
            before += code.ToString() + ", ";
    ...
    public void GetSortedDataBack(out uint[] _sortedKeys, out uint[] _sortedValues)
    {
        _sortedKeys = new uint[_dataLength];
        _keys.GetData(_sortedKeys);

        _sortedValues = new uint[_dataLength];
        _values.GetData(_sortedValues);
    }
